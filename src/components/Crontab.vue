      ...
      <div class="popup-result">
        ...
        <table>
          <thead>
            ...
            >{{Object.values(item)[0]}}</th>
            <th>crontab完整表达式</th>
          </thead>
          <tbody>
            <td v-for="item in showComponent" :key="item">
              <span>{{contabValueObj[item]}}</span>
            </td>
...
export default {
  ...
  methods: {
    ...
    showComponentControl() {
      const arr = Object.keys(this.contabValueObj)

      const newArr = arr.filter(this.shouldHide)
      this.showComponent = newArr
    },
    shouldHideCronResult() {
      ...
      newTableTitles.map((item, index) => {
        if (
          that.hideComponent &&
          that.hideComponent.includes(Object.keys(item)[0])
        ) {
    ...
    resolveExp() {
      ...
      if (this.expression) {
        ...
        if (arr.length >= 5) {
          ...
          for (let i in newObj) {
            if (newObj[i]) this.changeRadio(i, newObj[i])
          }
        }
      } else {
        //没有传入的表达式 则还原
